...
export class ChatController {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('room/:roomId')
  @ApiOperation({ summary: '특정 채팅방의 대화 전체 대화 조회'})
  async getRoomChats(@Param('roomId') roomId: number, @User() user) : Promise<{ statusCode: number, message: string, data: MessageEntity[]}>{
  const chats = await this.chatService.findAllChatByRoom(user.id, roomId);
  return {
    statusCode: 200,
